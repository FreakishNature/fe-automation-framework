...
@Log4j
public class ElementsList<T extends BaseElement> {
    ...
    public ElementsList(By locator, String name, BaseElement parentElement) {
    ...
    public <T extends BaseElement> Object apply(Class<T> clazz, Function<T, Object> func) {
    ...
    public <T extends BaseElement> T get(int index, Class<T> clazz) {
    ...
    public <T extends BaseElement> T get(int index, Class<T> clazz, BaseElement parent) {
        // xpath index starts from 1
        String elementLocator = "(" + Helpers.getRegexMatch(this.locator.toString(), xpathPattern, 0) + ")[" + ++index + "]";
        T object = null;
        try {
...
//        return Helpers.range(getCount()).stream().map(i -> this.getElement().get(i).getText().trim()).collect(Collectors.toList());
    ...
    public <T extends BaseElement> T getByContainingText(String s, Class<T> clazz) {
    ...
    public <T extends BaseElement> T getByContainingText(String s, Class<T> clazz, BaseElement parent) {
        String newLocator = Helpers.getRegexMatch(this.locator.toString(), xpathPattern, 0);
        if (!newLocator.isEmpty()) {
            newLocator = newLocator.replaceAll("]$", " and .//*[normalize-space()=\"" + s + "\"]]")
                    + " | "
    ...
    public <T extends BaseElement> T getByStrictText(String s, Class<T> clazz) {
        String newLocator = Helpers.getRegexMatch(this.locator.toString(), xpathPattern, 0);
        if (!newLocator.isEmpty()) {
            newLocator = newLocator.replaceAll("]$", " and .//*[normalize-space()=\"" + s + "\"]]")
                    + " | "
    ...
    public <T extends BaseElement> T getByStrictText(String s, Class<T> clazz, BaseElement parent) {
        String newLocator = Helpers.getRegexMatch(this.locator.toString(), xpathPattern, 0);
        if (!newLocator.isEmpty()) {
            newLocator = newLocator.replaceAll("]$", " and .//*[normalize-space()=\"" + s + "\"]]")
                    + " | "
